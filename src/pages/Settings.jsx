...
export default function Settings() {
  ...
  return (
    <div className="px-8 py-7 max-w-[900px] space-y-6">
      <div>
        <h1 className="text-xl font-bold text-primary dark:text-gray-100 mb-1">Settings</h1>
        <p className="text-sm text-gray-500 dark:text-gray-400">Defaults applied to every new invoice you create.</p>
      </div>
      ...
      <Card>
        ...
        <div className="space-y-4">
          <div className="flex items-center justify-between">
            ...
            <input type="number" value={settings.defaultVatPercent} onChange={(e) => updateSettings({ defaultVatPercent: e.target.value })} className="w-24 px-3 py-1.5 text-right rounded-lg border border-border dark:border-dark-border bg-white dark:bg-dark-bg text-sm outline-none focus:ring-2 focus:ring-accent/30" />
          </div>
          <div className="flex items-center justify-between">
            <Toggle label="Enable Tax by default" checked={settings.defaultTaxEnabled} onChange={(v) => updateSettings({ defaultTaxEnabled: v })} />
            <input type="number" value={settings.defaultTaxPercent} onChange={(e) => updateSettings({ defaultTaxPercent: e.target.value })} className="w-24 px-3 py-1.5 text-right rounded-lg border border-border dark:border-dark-border bg-white dark:bg-dark-bg text-sm outline-none focus:ring-2 focus:ring-accent/30" />
          </div>
        </div>
      </Card>
      ...
      <Card>
        <SectionTitle subtitle="Applied automatically to the payment section of every invoice">Default Payment Information</SectionTitle>
        <div className="grid grid-cols-2 gap-4">
          ...
          <Input label="Mobile Banking" className="col-span-2" value={settings.payment.mobileBanking} onChange={(e) => updatePayment({ mobileBanking: e.target.value })} />
          <Textarea label="Payment Notes" className="col-span-2" rows={3} value={settings.payment.notes} onChange={(e) => updatePayment({ notes: e.target.value })} />
        </div>
      </Card>

      <Card>
        <SectionTitle subtitle="Quick-pick suggestions while filling out a service table">Frequently Used Services</SectionTitle>
        <div className="flex flex-wrap gap-2 mb-4">
          {services.map((s) => (
            <span key={s} className="inline-flex items-center gap-1.5 px-3 py-1.5 rounded-full bg-gray-100 dark:bg-white/10 text-sm text-gray-700 dark:text-gray-200">
              {s}
              <button onClick={() => removeService(s)} className="text-gray-400 hover:text-red-500">×</button>
            </span>
          ))}
        </div>
        <div className="flex gap-2">
